...
- [ ] Audit all `threadgroup_barrier` usage (lines 50, 149, 226, 299, 368, 439, 499, 544, 610, 751, 947, 957, 964, 973, 986, 1057, 1067, 1106, 1116, 1148, 1161, 1225, 1311, 1402, 1415, 1468, 1544, 1623, 1689)
- [ ] Identify barriers that can be eliminated by fusing operations
- [ ] Evaluate using `simd_shuffle` for small reductions instead of threadgroup barriers
- [ ] Benchmark and tune `ZIGGY_MAX_ROW_SIMDGROUPS = 8` constant
- [ ] Benchmark and tune `ZIGGY_MAX_Q4K_SIMDGROUPS = 8` constant
- [ ] Evaluate if `ZIGGY_MAX_HEAD_DIM = 256` is optimal or can be dynamic

### Metal Shader: Attention Kernel Optimization (lines 882-997)
- [ ] Analyze the 3-stage reduction pattern (local max → threadgroup max → global max)
- [ ] Evaluate using `simd_max` and `simd_sum` more aggressively
- [ ] Test `threadgroup_barrier(mem_flags::mem_none)` where memory ordering isn't critical
- [ ] Consider making `local_out[ZIGGY_MAX_HEAD_DIM]` dynamically sized based on actual `head_dim`
- [ ] Profile attention kernel to identify actual bottleneck stages

### Metal Shader: RoPE Kernel Optimization (lines 800-880)
- [ ] Analyze `apply_rope_to_dst_f32` kernel per-thread trig computation
- [ ] Implement precomputation of `cos(theta)` and `sin(theta)` in shared memory per head
- [ ] Evaluate fast math approximations for trig functions if precision allows
- [ ] Benchmark interleaved vs neox rope styles for performance differences
...
- [ ] Run `ziggy-llm bench --metal-profile` on target models
- [ ] Establish baseline TPS measurements
- [ ] Identify top 3 bottleneck categories from profile output
...
- [ ] projections - Matrix-vector multiplication operations
- [ ] attention - Self-attention computation
- [ ] kv_writes - Key-value cache writes
- [ ] normalization - RMS norm operations
- [ ] elementwise_ops - Element-wise operations (SiLU, GeLU, etc.)
- [ ] output_reduce - Output layer reductions (argmax, sampling)
- [ ] commit_wait - GPU command buffer commit wait time
- [ ] host_readback - GPU to CPU memory transfers
- [ ] cpu_sampling - CPU-side sampling operations

### Batch Operations

#### Existing Batch Kernels
- [ ] Audit usage of `batchArgmaxPipeline` (bridge.m line 46)
- [ ] Audit usage of `batchMatvecAddPipeline` (bridge.m line 47)
- [ ] Audit usage of `batchSiluMulPipeline` (bridge.m line 58)
- [ ] Verify these are being used effectively
...
- [ ] Map all commit points in inference path
- [ ] Measure time between commits
- [ ] Identify commits that can be deferred
- [ ] Test batching multiple layer operations
- [ ] Find optimal batch size through benchmarking

#### Commit Statistics
- [ ] Use `commitSequenceTimed` stats (lines 308-324 in metal_backend.zig)
- [ ] Track CPU wait time vs GPU elapsed time
- [ ] Identify commits with high CPU wait overhead
