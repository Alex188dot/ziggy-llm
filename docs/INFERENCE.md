...
| 2026-04-10 | Cold `bench --bench-runs 5`, `temp 0`, GPU greedy argmax; MoonQuant RMS-entry fused Q/KV/K             | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |   1026.318 |  1121.589 | 2250.819 |    128.420 |
| 2026-04-10 | Warm avg (`4` reused runs), `temp 0`, GPU greedy argmax; MoonQuant RMS-entry fused Q/KV/K              | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |      0.140 |   157.026 |  259.205 |    130.875 |
| 2026-04-10 | Cold `bench --bench-runs 5`, `temp 0`, GPU greedy argmax; plus FFN RMS-entry fused gate/up fanout      | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |   1089.052 |  1250.174 | 2444.196 |    127.119 |
| 2026-04-10 | Warm avg (`4` reused runs), `temp 0`, GPU greedy argmax; plus FFN RMS-entry fused gate/up fanout       | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |      0.152 |   156.656 |  261.163 |    133.859 |
| 2026-04-11 | Cold `bench --bench-runs 5`, `temp 0`, GPU greedy argmax; clean rebuild baseline after mixed-state fix | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |   1143.448 |  1299.133 | 2546.295 |    128.980 |
| 2026-04-11 | Warm avg (`4` reused runs), `temp 0`, GPU greedy argmax; clean rebuild baseline after mixed-state fix  | MacBook Pro M3 18GB | `metal` | `tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` |            20 |              128 |      0.156 |   152.853 |  256.207 |    134.496 |
...
- The 2026-04-09 profiling series now shows that `commit_wait` alone is no longer enough evidence to call the decode loop “CPU-orchestration-bound.” After commit-path instrumentation was added, TinyLlama warm decode measured `125.185 TPS` with `996.927 ms` of `commit_wait` out of `1023.226 ms` total (`97.430%`), but that split into `968.863 ms` of GPU time and only `28.064 ms` of non-GPU wait. The runtime is already at `1.000` command buffer per generated token, so `pendingCommandBuffer` / `beginSequence` / `commitSequenceTimed` are no longer the main limiter on this path.
- The optimization frontier has therefore shifted. The next likely decode wins must come primarily from reducing real GPU work per token: fewer kernels and encoder boundaries inside the single per-token command buffer, fewer global-memory passes, deeper decode-specific fusion, more aggressive specialization for the hot TinyLlama and Llama shapes, and possibly a narrower persistent decode-step kernel if the modular fusion path stalls.
- The later 2026-04-09 re-run with fused decode-path profiling is consistent with that conclusion. TinyLlama warm decode still showed heavy steady-state GPU work with `26624` encoders and `23808` dispatches total, or `208` encoders and `186` dispatches per generated token. The fused-path counters show `Q+RoPE` active on all eligible layers, `KV half-write` active on `1776 / 3256` attempts (`54.545%`), and the remaining KV cases falling back only because `attn_v` is not `tensor_type=12`, after which the single fused `K` path still succeeds on all `1480` attempts.
- The 2026-04-10 MoonQuant RMS-entry fusion pass improved warm TinyLlama decode from `128.176 TPS` to `130.875 TPS` while keeping fused `Q+RoPE` and fused `KV half-write` at `100%` success. Dispatches stayed flat at `210` per token, so this gain came from removing extra GPU work inside existing dispatches rather than from launch-count reduction.
- The follow-up 2026-04-10 FFN RMS-entry fused gate/up fanout pass pushed warm TinyLlama decode to `133.859 TPS`. Dispatches still stayed flat at `210` per token, but warm decode time fell to `956.232 ms` and warm normalization time fell again to `3.584 ms`, which is consistent with the FFN entry path no longer materializing a separate normalized FFN input before the fused down-add stage.
- The 2026-04-11 clean-rebuild baseline restored compile correctness after the mixed `llama_gpu.zig` merge state and slightly improved warm TinyLlama decode again, reaching `134.496 TPS` from `133.859 TPS`. Dispatches stayed flat at `188` per token and the fused-path counters remained `100%` for fused `Q+RoPE` and fused `KV half-write`, so this was a small steady-state improvement rather than a new launch-count reduction.
...
- The 2026-04-09 Llama 3.2 run remains the clearest current Phase 5 data point for CPU-side overhead. Warm decode stayed near the documented baseline at `41.790 TPS`, but unlike TinyLlama it used `cpu-logits` sampling and spent `77.857 ms` in CPU sampling plus `1.356 ms` in host readback during decode. That overhead is measurable, but it is still secondary to the GPU-side decode workload. The fused-path counters again show `Q+RoPE` active everywhere, with fused `KV half-write` active on `1890 / 3780` attempts (`50%`) and fused `K` succeeding on the remaining `1890` attempts. The practical takeaway is that Phase 6 should now focus on proving which CPU-side costs are still worth attacking, while Phase 3 through Phase 5 become the more likely source of material throughput gains.

## Benchmark Table — Llama 3.1 8B

| Date       | Mode                                                | Machine             | Backend | Model                                    | Prompt tokens | Generated tokens | Startup ms | Prompt ms |   TTFT ms | Decode TPS |
| ---------- | --------------------------------------------------- | ------------------- | ------- | ---------------------------------------- | ------------: | ---------------: | ---------: | --------: | --------: | ---------: |
| 2026-04-06 | Cold `bench --bench-runs 5`, `temp 0.7`, CPU logits | MacBook Pro M3 18GB | `metal` | `Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf` |             7 |              128 |  11227.159 |  1647.057 | 12876.146 |     18.296 |
| 2026-04-06 | Warm avg (`4` reused runs), `temp 0.7`, CPU logits  | MacBook Pro M3 18GB | `metal` | `Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf` |             7 |              128 |      0.045 |   364.991 |   365.882 |     18.120 |

## Benchmark Table — Qwen3 8B
