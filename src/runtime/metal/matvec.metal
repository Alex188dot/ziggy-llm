...
#define ZIGGY_Q4K_DUAL_RMS_KERNEL(NAME, STATIC_COLS) \
kernel void NAME( \
    device const uchar *matrix0 [[buffer(0)]], \
    device const uchar *matrix1 [[buffer(1)]], \
    device const float *input [[buffer(2)]], \
...
ZIGGY_Q4K_DUAL_RMS_KERNEL(matvec_q4k_dual_rms_f32, 0)
ZIGGY_Q4K_DUAL_RMS_KERNEL(matvec_q4k_dual_rms_2048_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_0)
ZIGGY_Q4K_DUAL_RMS_KERNEL(matvec_q4k_dual_rms_5632_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_1)

#undef ZIGGY_Q4K_DUAL_RMS_KERNEL
...
#define ZIGGY_Q4K_QK_HALF_RMS_KERNEL(NAME, STATIC_COLS) kernel void NAME(     device const uchar *matrix_q [[buffer(0)]],     device const uchar *matrix_k [[buffer(1)]],     device const float *input [[buffer(2)]],     device const float *norm_weights [[buffer(3)]],     device const float *norm_scale [[buffer(4)]],     device float *q_output [[buffer(5)]],     device half *k_cache [[buffer(6)]],     constant uint &dst_base [[buffer(7)]],     constant uint &head_count [[buffer(8)]],     constant uint &head_dim [[buffer(9)]],     constant uint &pair_count [[buffer(10)]],     constant uint &cols [[buffer(11)]],     constant uint &position [[buffer(12)]],     constant float &freq_base [[buffer(13)]],     uint row_pair [[threadgroup_position_in_grid]],     uint lane [[thread_index_in_threadgroup]],     uint simd_lane [[thread_index_in_simdgroup]],     uint simd_group [[simdgroup_index_in_threadgroup]],     uint threads_per_group [[threads_per_threadgroup]],     uint threads_per_simdgroup [[threads_per_simdgroup]] ) {     if (row_pair >= head_count * pair_count) return;     constexpr uint kStaticCols = STATIC_COLS;     const uint effective_cols = kStaticCols == 0 ? cols : kStaticCols;     if (kStaticCols != 0 && cols != kStaticCols) return;     const float input_scale = norm_scale[0];     const uint head = row_pair / pair_count;     const uint pair = row_pair % pair_count;     const uint row0 = head * head_dim + pair * 2;     const uint row1 = row0 + 1;     const uint blocks_per_row = effective_cols / ZIGGY_Q4K_VALUES_PER_BLOCK;     const uint row_stride = blocks_per_row * ZIGGY_Q4K_BYTES_PER_BLOCK;     const device uchar *q_row_bytes0 = matrix_q + row0 * row_stride;     const device uchar *q_row_bytes1 = matrix_q + row1 * row_stride;     const device uchar *k_row_bytes0 = matrix_k + row0 * row_stride;     const device uchar *k_row_bytes1 = matrix_k + row1 * row_stride;     const uint packed_chunks_per_row = blocks_per_row * ZIGGY_Q4K_GROUPS_PER_BLOCK * ZIGGY_Q4K_PACKED_CHUNKS_PER_GROUP;     threadgroup float partial_q0[ZIGGY_MAX_Q4K_SIMDGROUPS];     threadgroup float partial_q1[ZIGGY_MAX_Q4K_SIMDGROUPS];     threadgroup float partial_k0[ZIGGY_MAX_Q4K_SIMDGROUPS];     threadgroup float partial_k1[ZIGGY_MAX_Q4K_SIMDGROUPS];     float local_q0 = 0.0f, local_q1 = 0.0f, local_k0 = 0.0f, local_k1 = 0.0f;     for (uint chunk_index = lane; chunk_index < packed_chunks_per_row; chunk_index += threads_per_group) {         const uint block_group_index = chunk_index / ZIGGY_Q4K_PACKED_CHUNKS_PER_GROUP;         const uint chunk_in_group = chunk_index % ZIGGY_Q4K_PACKED_CHUNKS_PER_GROUP;         const uint block_index = block_group_index / ZIGGY_Q4K_GROUPS_PER_BLOCK;         const uint group = block_group_index % ZIGGY_Q4K_GROUPS_PER_BLOCK;         const uint q_offset = chunk_in_group * 4;         const uint input_offset = block_index * ZIGGY_Q4K_VALUES_PER_BLOCK + group * ZIGGY_Q4K_VALUES_PER_GROUP + q_offset;         const float4 input_low = ziggy_normed_value4(input, norm_weights, input_scale, input_offset);         const float4 input_high = ziggy_normed_value4(input, norm_weights, input_scale, input_offset + 32);         const uint scale_index = group * 2;         { const device uchar *block = q_row_bytes0 + block_index * ZIGGY_Q4K_BYTES_PER_BLOCK; const float d = read_half_le(block, 0); const float dmin = read_half_le(block, 2); const device uchar *scales = block + 4; const device uchar *q = block + 16 + group * ZIGGY_Q4K_PACKED_BYTES_PER_GROUP + q_offset; const uchar4 packed = uchar4(q[0], q[1], q[2], q[3]); const uchar4 low_q = packed & uchar4(0x0F); const uchar4 high_q = packed >> 4; const float d1 = d * float(get_scale_k4(scales, scale_index + 0)); const float m1 = dmin * float(get_min_k4(scales, scale_index + 0)); const float d2 = d * float(get_scale_k4(scales, scale_index + 1)); const float m2 = dmin * float(get_min_k4(scales, scale_index + 1)); local_q0 += dot(float4(low_q) * d1 - float4(m1), input_low); local_q0 += dot(float4(high_q) * d2 - float4(m2), input_high); }         { const device uchar *block = q_row_bytes1 + block_index * ZIGGY_Q4K_BYTES_PER_BLOCK; const float d = read_half_le(block, 0); const float dmin = read_half_le(block, 2); const device uchar *scales = block + 4; const device uchar *q = block + 16 + group * ZIGGY_Q4K_PACKED_BYTES_PER_GROUP + q_offset; const uchar4 packed = uchar4(q[0], q[1], q[2], q[3]); const uchar4 low_q = packed & uchar4(0x0F); const uchar4 high_q = packed >> 4; const float d1 = d * float(get_scale_k4(scales, scale_index + 0)); const float m1 = dmin * float(get_min_k4(scales, scale_index + 0)); const float d2 = d * float(get_scale_k4(scales, scale_index + 1)); const float m2 = dmin * float(get_min_k4(scales, scale_index + 1)); local_q1 += dot(float4(low_q) * d1 - float4(m1), input_low); local_q1 += dot(float4(high_q) * d2 - float4(m2), input_high); }         { const device uchar *block = k_row_bytes0 + block_index * ZIGGY_Q4K_BYTES_PER_BLOCK; const float d = read_half_le(block, 0); const float dmin = read_half_le(block, 2); const device uchar *scales = block + 4; const device uchar *q = block + 16 + group * ZIGGY_Q4K_PACKED_BYTES_PER_GROUP + q_offset; const uchar4 packed = uchar4(q[0], q[1], q[2], q[3]); const uchar4 low_q = packed & uchar4(0x0F); const uchar4 high_q = packed >> 4; const float d1 = d * float(get_scale_k4(scales, scale_index + 0)); const float m1 = dmin * float(get_min_k4(scales, scale_index + 0)); const float d2 = d * float(get_scale_k4(scales, scale_index + 1)); const float m2 = dmin * float(get_min_k4(scales, scale_index + 1)); local_k0 += dot(float4(low_q) * d1 - float4(m1), input_low); local_k0 += dot(float4(high_q) * d2 - float4(m2), input_high); }         { const device uchar *block = k_row_bytes1 + block_index * ZIGGY_Q4K_BYTES_PER_BLOCK; const float d = read_half_le(block, 0); const float dmin = read_half_le(block, 2); const device uchar *scales = block + 4; const device uchar *q = block + 16 + group * ZIGGY_Q4K_PACKED_BYTES_PER_GROUP + q_offset; const uchar4 packed = uchar4(q[0], q[1], q[2], q[3]); const uchar4 low_q = packed & uchar4(0x0F); const uchar4 high_q = packed >> 4; const float d1 = d * float(get_scale_k4(scales, scale_index + 0)); const float m1 = dmin * float(get_min_k4(scales, scale_index + 0)); const float d2 = d * float(get_scale_k4(scales, scale_index + 1)); const float m2 = dmin * float(get_min_k4(scales, scale_index + 1)); local_k1 += dot(float4(low_q) * d1 - float4(m1), input_low); local_k1 += dot(float4(high_q) * d2 - float4(m2), input_high); }     }     const float sum_q0 = simd_sum(local_q0); const float sum_q1 = simd_sum(local_q1); const float sum_k0 = simd_sum(local_k0); const float sum_k1 = simd_sum(local_k1);     if (simd_lane == 0) { partial_q0[simd_group] = sum_q0; partial_q1[simd_group] = sum_q1; partial_k0[simd_group] = sum_k0; partial_k1[simd_group] = sum_k1; }     threadgroup_barrier(mem_flags::mem_threadgroup);     if (lane == 0) {         float q0 = 0.0f, q1 = 0.0f, k0 = 0.0f, k1 = 0.0f;         const uint simd_group_count = (threads_per_group + threads_per_simdgroup - 1) / threads_per_simdgroup;         for (uint index = 0; index < simd_group_count; index += 1) { q0 += partial_q0[index]; q1 += partial_q1[index]; k0 += partial_k0[index]; k1 += partial_k1[index]; }         const float exponent = float(pair * 2) / float(pair_count * 2);         const float theta = float(position) / pow(freq_base, exponent);         const float cos_theta = cos(theta);         const float sin_theta = sin(theta);         q_output[row0] = q0 * cos_theta - q1 * sin_theta;         q_output[row1] = q0 * sin_theta + q1 * cos_theta;         k_cache[dst_base + row0] = half(k0 * cos_theta - k1 * sin_theta);         k_cache[dst_base + row1] = half(k0 * sin_theta + k1 * cos_theta);     } }

ZIGGY_Q4K_QK_HALF_RMS_KERNEL(matvec_q4k_qk_half_rms_f32, 0)
ZIGGY_Q4K_QK_HALF_RMS_KERNEL(matvec_q4k_qk_half_rms_2048_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_0)
ZIGGY_Q4K_QK_HALF_RMS_KERNEL(matvec_q4k_qk_half_rms_5632_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_1)

#undef ZIGGY_Q4K_QK_HALF_RMS_KERNEL
...
#define ZIGGY_MOONQ_Q4K_DUAL_RMS_KERNEL(NAME, STATIC_COLS) \
kernel void NAME( \
    device const uchar *matrix0 [[buffer(0)]], \
    device const uchar *matrix1 [[buffer(1)]], \
    device const float *input [[buffer(2)]], \
...
ZIGGY_MOONQ_Q4K_DUAL_RMS_KERNEL(matvec_moonq_q4k_dual_rms_f32, 0)
ZIGGY_MOONQ_Q4K_DUAL_RMS_KERNEL(matvec_moonq_q4k_dual_rms_2048_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_0)
ZIGGY_MOONQ_Q4K_DUAL_RMS_KERNEL(matvec_moonq_q4k_dual_rms_5632_f32, ZIGGY_MOONQ_Q4K_SPECIAL_COLS_1)

#undef ZIGGY_MOONQ_Q4K_DUAL_RMS_KERNEL
...
kernel void batch_matvec_f32_all(
    device const float *matrix [[buffer(0)]],
    device const float *input [[buffer(1)]],
    device float *output [[buffer(2)]],
    constant uint &rows [[buffer(3)]],
    ...
    uint row_batch [[threadgroup_position_in_grid]],
    uint lane [[thread_index_in_threadgroup]],
    uint simd_lane [[thread_index_in_simdgroup]],
    uint simd_group [[simdgroup_index_in_threadgroup]],
    uint threads_per_group [[threads_per_threadgroup]],
    ...
    if (rows == 0) return;
    const uint total_rows = rows * batch_count;
    if (row_batch >= total_rows) return;
    const uint batch_idx = row_batch / rows;
    const uint row = row_batch % rows;
...
#define ZIGGY_BATCH_Q4K_MATVEC_ALL_KERNEL(NAME, STATIC_COLS) \
kernel void NAME( \
    device const uchar *matrix [[buffer(0)]], \
    device const float *input [[buffer(1)]], \
    device float *output [[buffer(2)]], \
...
ZIGGY_BATCH_Q4K_MATVEC_ALL_KERNEL(batch_matvec_q4k_f32_all, 0)

#undef ZIGGY_BATCH_Q4K_MATVEC_ALL_KERNEL

kernel void batch_matvec_q6k_f32_all(
    device const uchar *matrix [[buffer(0)]],
    device const float *input [[buffer(1)]],
    device float *output [[buffer(2)]],
    constant uint &rows [[buffer(3)]],
    ...
    for (uint chunk_index = lane; chunk_index < chunks_per_row; chunk_index += threads_per_group) {
        ...
        local_sum += s0 * q1 * batch_input[input_offset + 0];
        local_sum += s2 * q2 * batch_input[input_offset + 32];
        local_sum += s4 * q3 * batch_input[input_offset + 64];
        local_sum += s6 * q4 * batch_input[input_offset + 96];
    }
...
kernel void batch_silu_mul_f32_all(
    device float *gate [[buffer(0)]],
    device const float *up [[buffer(1)]],
    constant uint &count [[buffer(2)]],
    constant uint &batch_count [[buffer(3)]],
    ...
    const uint total = count * batch_count;
...
kernel void batch_add_in_place_f32_all(
    device float *dst [[buffer(0)]],
    device const float *src [[buffer(1)]],
    constant uint &count [[buffer(2)]],
    constant uint &batch_count [[buffer(3)]],
...
kernel void batch_rms_norm_f32_all(
    device const float *input [[buffer(0)]],
    device const float *weights [[buffer(1)]],
    device float *output [[buffer(2)]],
    constant uint &count [[buffer(3)]],
