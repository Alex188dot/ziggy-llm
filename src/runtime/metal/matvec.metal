...
kernel void matvec_q6k_f32(
    ...
    for (uint chunk_index = lane; chunk_index < chunks_per_row; chunk_index += threads_per_group) {
        ...
        const float s0 = d * float(as_type<char>(scales[group16 + 0]));
        const float s2 = d * float(as_type<char>(scales[group16 + 2]));
        const float s4 = d * float(as_type<char>(scales[group16 + 4]));
        const float s6 = d * float(as_type<char>(scales[group16 + 6]));
