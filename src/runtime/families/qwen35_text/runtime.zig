...
fn qwen35TextDeinit(ctx: ?*anyopaque) void {
    _ = ctx;
}

pub fn createQwen35TextFamilyRuntime() families_mod.FamilyRuntime {
    return families_mod.FamilyRuntime{
        ...
        .generate_fn = struct {
            fn f(ctx: ?*anyopaque, allocator: std.mem.Allocator, model_path: []const u8, prompt: []const u8, options: families_mod.FamilyGenerateOptions) !families_mod.FamilyReport {
                return qwen35TextGenerate(ctx, allocator, model_path, prompt, options);
            }
        }.f,
        .deinit_fn = qwen35TextDeinit,
    };
}

pub fn createQwen35TextFamilyHandler() families_mod.FamilyHandler {
    return families_mod.FamilyHandler{
        ...
        .runtime = createQwen35TextFamilyRuntime(),
    };
}
