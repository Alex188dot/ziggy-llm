...
fn gemmaDeinit(ctx: ?*anyopaque) void {
    _ = ctx;
}

pub fn createGemmaFamilyRuntime() families_mod.FamilyRuntime {
    return families_mod.FamilyRuntime{
        ...
        .generate_fn = struct {
            fn f(ctx: ?*anyopaque, allocator: std.mem.Allocator, model_path: []const u8, prompt: []const u8, options: families_mod.FamilyGenerateOptions) !families_mod.FamilyReport {
                return gemmaGenerate(ctx, allocator, model_path, prompt, options);
            }
        }.f,
        .deinit_fn = gemmaDeinit,
    };
}

pub fn createGemmaFamilyHandler() families_mod.FamilyHandler {
    return families_mod.FamilyHandler{
        ...
        .runtime = createGemmaFamilyRuntime(),
    };
}
