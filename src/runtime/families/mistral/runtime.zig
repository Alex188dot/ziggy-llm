...
fn mistralDeinit(ctx: ?*anyopaque) void {
    _ = ctx;
}

pub fn createMistralFamilyRuntime() families_mod.FamilyRuntime {
    return families_mod.FamilyRuntime{
        ...
        .generate_fn = struct {
            fn f(ctx: ?*anyopaque, allocator: std.mem.Allocator, model_path: []const u8, prompt: []const u8, options: families_mod.FamilyGenerateOptions) !families_mod.FamilyReport {
                return mistralGenerate(ctx, allocator, model_path, prompt, options);
            }
        }.f,
        .deinit_fn = mistralDeinit,
    };
}

pub fn createMistralFamilyHandler() families_mod.FamilyHandler {
    return families_mod.FamilyHandler{
        ...
        .runtime = createMistralFamilyRuntime(),
    };
}
