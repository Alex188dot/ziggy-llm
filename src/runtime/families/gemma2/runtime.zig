...
fn gemma2Deinit(ctx: ?*anyopaque) void {
    _ = ctx;
}

pub fn createGemma2FamilyRuntime() families_mod.FamilyRuntime {
    return families_mod.FamilyRuntime{
        ...
        .generate_fn = struct {
            fn f(ctx: ?*anyopaque, allocator: std.mem.Allocator, model_path: []const u8, prompt: []const u8, options: families_mod.FamilyGenerateOptions) !families_mod.FamilyReport {
                return gemma2Generate(ctx, allocator, model_path, prompt, options);
            }
        }.f,
        .deinit_fn = gemma2Deinit,
    };
}

pub fn createGemma2FamilyHandler() families_mod.FamilyHandler {
    return families_mod.FamilyHandler{
        ...
        .runtime = createGemma2FamilyRuntime(),
    };
}
