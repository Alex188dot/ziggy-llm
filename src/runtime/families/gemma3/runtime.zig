...
fn gemma3Deinit(ctx: ?*anyopaque) void {
    _ = ctx;
}

pub fn createGemma3FamilyRuntime() families_mod.FamilyRuntime {
    return families_mod.FamilyRuntime{
        ...
        .generate_fn = struct {
            fn f(ctx: ?*anyopaque, allocator: std.mem.Allocator, model_path: []const u8, prompt: []const u8, options: families_mod.FamilyGenerateOptions) !families_mod.FamilyReport {
                return gemma3Generate(ctx, allocator, model_path, prompt, options);
            }
        }.f,
        .deinit_fn = gemma3Deinit,
    };
}

pub fn createGemma3FamilyHandler() families_mod.FamilyHandler {
    return families_mod.FamilyHandler{
        ...
        .runtime = createGemma3FamilyRuntime(),
    };
}
