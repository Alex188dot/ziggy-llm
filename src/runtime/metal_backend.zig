...
pub fn writeShortlistEntries(buffer: BufferHandle, values: []const ShortlistEntry) !void {
    if (!build_enabled_value) return error.MetalDisabled;
    if (values.len * @sizeOf(ShortlistEntry) > buffer.byte_len) return error.MetalBufferError;
    try writeBufferBytes(buffer.raw, std.mem.sliceAsBytes(values));
}
...
pub fn indexedMatvecQ3K(
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_indexed_matvec_q3_k_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn dualIndexedMatvecQ3K(
    ...
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_dual_indexed_matvec_q3_k_f32(
        state.context,
        matrix_a_buffer.raw,
        matrix_b_buffer.raw,
        input.raw,
...
pub fn indexedMatvecQ3KAddWeighted(
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_indexed_matvec_q3_k_add_weighted_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
