...
pub fn getOrCreateBufferForF32(backend: backend_api.MatVecBackend, values: []const f32) !BufferHandle {
    if (!build_enabled_value) return error.MetalDisabled;
    const state = stateFromCtx(backend.ctx);
    return state.matrixBuffer(values);
}
...
pub fn runMatVecQ5KToBuffer(
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q5k_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn runMatVecQ5KToDstBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    output: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q5k_f32_to_dst(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn runMatVecQ5KAddToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    output: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q5k_add_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn runMatVecQ3KToBuffer(
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q3k_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn runMatVecQ3KToDstBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    output: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q3k_f32_to_dst(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn runMatVecQ3KAddToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    output: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q3k_add_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn indexedMatvecQ3K(
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_indexed_matvec_q3k_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn dualIndexedMatvecQ3K(
    ...
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_dual_indexed_matvec_q3k_f32(
        state.context,
        matrix_a_buffer.raw,
        matrix_b_buffer.raw,
        input.raw,
...
pub fn indexedMatvecQ3KAddWeighted(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    output: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_indexed_matvec_q3k_add_weighted_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        output.raw,
...
pub fn applyRoPEAndStoreToCache(
    backend: backend_api.MatVecBackend,
    src: BufferHandle,
    dst: BufferHandle,
    dst_offset_bytes: usize,
    ...
    const state = stateFromCtx(backend.ctx);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_apply_rope_and_store_to_cache_f32(
        state.context,
        src.raw,
        dst.raw,
        dst_offset_bytes,
...
pub fn runMatVecStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix: []const f32,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    if (!build_enabled_value) return error.MetalDisabled;
    if (input.byte_len < cols * @sizeOf(f32)) return error.MetalBufferError;
    const state = stateFromCtx(backend.ctx);
    const matrix_buffer = try state.matrixBuffer(matrix[0 .. rows * cols]);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
        cache_offset_elements,
        @intCast(rows),
        @intCast(cols),
        &error_buf,
        error_buf.len,
...
pub fn runMatVecQ4KStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q4k_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn runMatVecQ5KStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q5k_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn runMatVecQ6KStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q6k_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn runMatVecQ8_0StoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q8_0_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn runMatVecQ3KStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_q3k_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn runMatVecMoonQ4KStoreKvToBuffer(
    backend: backend_api.MatVecBackend,
    matrix_bytes: []const u8,
    input: BufferHandle,
    cache: BufferHandle,
    ...
    const matrix_buffer = try state.rawBuffer(matrix_bytes);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_run_matvec_moonq_q4k_store_kv_f32(
        state.context,
        matrix_buffer.raw,
        input.raw,
        cache.raw,
...
pub fn linearConv1dF32(
    backend: backend_api.MatVecBackend,
    qkv: BufferHandle,
    conv_state: BufferHandle,
    conv_weights: BufferHandle,
    conv_out: BufferHandle,
    layer_index: u32,
    block_count: u32,
    kernel_dim: u32,
    qkv_dim: u32,
    ...
    const state = stateFromCtx(backend.ctx);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_linear_conv1d_f32(
        state.context,
        qkv.raw,
        conv_state.raw,
        conv_weights.raw,
...
pub fn linearRecurrentNormF32(
    backend: backend_api.MatVecBackend,
    conv_out: BufferHandle,
    recurrent_state: BufferHandle,
    z: BufferHandle,
    a: BufferHandle,
    b: BufferHandle,
    dt_bias: BufferHandle,
    A_log: BufferHandle,
    ...
    const state = stateFromCtx(backend.ctx);
    var error_buf: [err_buf_len]u8 = std.mem.zeroes([err_buf_len]u8);
    try mapStatus(c.ziggy_metal_linear_recurrent_norm_f32(
        state.context,
        conv_out.raw,
        recurrent_state.raw,
        z.raw,
